...
export const MinimalRequestCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <>
      <div className={`w-60 pb-2.5 bg-white rounded-xl shadow outline outline-offset-[-1px] outline-gray-100 inline-flex flex-col justify-start items-center gap-2.5 overflow-hidden ${isDragging ? 'ring-2 ring-sky-600' : ''}`}>
        <Image
        ...
        <div className="self-stretch px-5 py-0.5 border-b border-gray-200 inline-flex justify-between items-center">
          <div className="w-48 flex justify-start items-center">
            <div className="pb-[0.67px] flex justify-center items-center gap-6">
              <div className="w-28 justify-center text-black text-base font-semibold font-['Segoe_UI']">{
                req.serviceName
                  ? req.serviceName.replace(/\w\S*/g, (w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
                  : ''
              }</div>
              <div className="justify-center text-sky-700 text-base font-bold font-['Segoe_UI']">{req.price} ETB</div>
            </div>
          </div>
        </div>
        <div className="self-stretch px-5 inline-flex justify-between items-center">
          <div className="flex-1 flex justify-start items-center">
            <div className="pb-[0.67px] flex justify-center items-center gap-6">
              <div className="justify-center text-black text-sm font-normal font-['Segoe_UI'] line-clamp-2 break-words">{req.description ? req.description.charAt(0).toUpperCase() + req.description.slice(1) : ''}</div>
            </div>
          </div>
        </div>
        <div className="px-5 w-full">
        <div className="px-5 py-2 bg-gray-50 rounded-md flex flex-col justify-center items-start gap-2">
        ...
        <button
          className="cursor-pointer w-52 p-1.5 rounded-[10px] outline outline-offset-[-1px] outline-sky-600 inline-flex justify-center items-start gap-14"
          onClick={onViewDetails}
          type="button"
        >
          <div className="justify-center text-sky-600 text-xs font-normal font-['Inter']">View</div>
        </button>
      </div>
    </>
  );
