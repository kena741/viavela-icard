...
export default function MobileSidebar({
    ...
    return (
        <>
            <div className={`lg:hidden fixed inset-0 z-40 transition-opacity ${open ? "opacity-100 pointer-events-auto bg-black/40" : "opacity-0 pointer-events-none"}`} onClick={onClose} aria-hidden={!open} />
            <aside className={`lg:hidden fixed top-0 right-0 z-50 h-full w-64 bg-white border-l border-gray-200 shadow-xl transform transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`} role="dialog" aria-modal="true">
                ...
                <div className="flex flex-col items-center gap-2 py-6 border-b border-gray-200">
                    <button
                        ...
                        className="relative h-20 w-20 rounded-full overflow-hidden border-2 border-sky-600 focus:outline-none"
                        aria-label="Change profile picture"
                        title="Change profile picture"
                    >
                        <Image src={profileImage || "/img/logo.svg"} alt="Profile" fill className="object-cover" />
                    </button>
                    <label htmlFor="mobile-profile-upload" className="text-xs text-sky-600 cursor-pointer hover:underline">Change picture</label>
