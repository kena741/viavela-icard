...
export function Sidebar() {
    ...
    return (
        <div className='hidden md:block'>
            <div
                data-sidebar="group"
                className="hidden md:flex flex-col w-64 bg-white border-r border-r-orange-500 fixed left-0 z-30 h-screen py-2.5 top-0 overflow-y-auto shadow-xl"
            >
                ...
                <div className="flex flex-col flex-1 overflow-y-auto px-4 pt-8">
                            ...
                            {navItems.map((item) => {
                                ...
                                if (item.subItems) {
                                    ...
                                    return (
                                        <li data-sidebar="menu-item" className="group/menu-item relative" key={item.href}>
                                            <button
                                                type="button"
                                                className={`peer/menu-button flex w-full text-sm font-normal font-['Segoe_UI'] leading-tight items-center gap-2 overflow-hidden rounded-md p-2 text-left outline-none ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 active:bg-orange-900 active:text-orange-400 disabled:pointer-events-none disabled:opacity-50 h-8 ${isActive || dropdownOpen
                                                    ? 'text-orange-400'
                                                    : 'text-gray-900 hover:bg-gray-100 hover:text-orange-400'
                                                    }`}
                                                onClick={() => setOpenDropdown((prev) => ({ ...prev, [item.label]: prev[item.label] ? false : true }))}
                                            >
                                                <span className={isActive || dropdownOpen ? 'text-orange-400' : 'text-orange-500'}>
                                                        ...
                                                        return (
                                                            <Link
                                                                ...
                                                                className={`w-full flex items-center justify-between p-2 text-sm rounded-md transition-all duration-200 ${isSubActive
                                                                    ? 'text-orange-400'
                                                                    : 'text-gray-800 hover:bg-gray-100 hover:text-orange-400'
                                                                    }`}
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    <span className={isSubActive ? 'text-orange-400' : 'text-orange-500'}>
                                                                        {!isSubActive && sub.inactiveIcon != null ? sub.inactiveIcon : sub.icon}
                                                                    </span>
                                                                    <span className={`${isSubActive ? 'text-orange-400' : 'text-gray-900'} font-medium transition-all duration-500 ease-out`}>{sub.label}</span>
                                                                </div>
                                                            </Link>
                                                        );
                                                    })}
                                ...
                                return (
                                    <li data-sidebar="menu-item" className="group/menu-item relative" key={item.href}>
                                        <Link
                                            href={item.href}
                                            className={`peer/menu-button flex text-sm font-normal font-['Segoe_UI'] leading-tight w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left outline-none ring-sidebar-ring transition-[width,height,padding] transition-all duration-500 ease-out focus-visible:ring-2 active:bg-orange-900 active:text-orange-400 disabled:pointer-events-none disabled:opacity-50 h-8 ${isActive ? 'text-orange-400' : 'text-gray-900 hover:bg-gray-100 hover:text-orange-400'}`}
                                        >
                                            <span className={isActive ? 'text-orange-400' : 'text-orange-500'}>
                                                {typeof item.icon === 'string' ? (
                    ...
                    <div className="mt-auto flex flex-col items-center text-xs text-orange-200 py-4">
                        <div className="mb-1">
                            {COPYRIGHT_TEXT}
                        </div>
                        <div>
